...
intersect <- intersect(rownames(umap), pheno$basename)
pheno <- pheno[match(intersect, pheno$basename),]
umap <- umap[match(intersect, rownames(umap)),]
pheno$umap1 <- umap[,1]
pheno$umap2 <- umap[,2]
...
# find any columns consisting of only one observation
unique <- ""

for (i in ncol(pheno)){
  if(length(unique(pheno[,1])) < 2
}
