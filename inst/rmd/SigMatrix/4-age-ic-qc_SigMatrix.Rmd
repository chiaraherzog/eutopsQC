...
if(any(grepl("_BC11", rownames(beta)))){
...
if(any(grepl("_BC11", rownames(beta)))){
  ...
  if(exists("pheno") && any(colnames(pheno) == "age")){
    ...
    pheno$widage <- WID_clocks(beta)$WID_general_clock 
    ...
    widage <- WID_clocks(beta)$WID_general_clock 
...
if(any(grepl("_BC11", rownames(beta)))){
  ...
  print("Mouse array - NA")
    ...
    if(exists("pheno") && any(colnames(pheno) == "smoking") && "cg16867657" %in% rownames(beta)){
    intersect <- intersect(colnames(beta), pheno$basename)
    pheno <- pheno[match(intersect, pheno$basename),]
    beta <- beta[,match(intersect, colnames(beta))]
    pheno$index <- as.numeric(beta["cg16867657",])
    plot <- pheno |>
